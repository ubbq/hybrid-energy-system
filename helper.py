...
def build_dg_model(price_data, wind_data, demand_data, 
                   turbine_data, horizon_length, include_grid=False):
    ...
    generators = filter(lambda x: device_control_parameters[x],
                         device_control_parameters)
    ...
    m.G = pyo.Set(initialize=generators, 
                        doc="All online generators defined by input")
    # --------------------------- Parameters ---------------------------------
    # Square root of round trip efficiency
    m.sqrteta_c = pyo.Param(initialize = np.sqrt(0.88), mutable=False,
                            doc="Square root of round trip efficiency charge")
    m.sqrteta_d = pyo.Param(initialize = np.sqrt(0.96), mutable=False,
                            doc="Square root of round trip efficiency discharge")

    # Parametric device start stop prep off
    m.pDeviceOn = pyo.Param(m.G, initialize=1, domain=pyo.Binary,mutable=False, \
                            doc="On/Off status of generator dev at time t")  
    m.pPrice    = pyo.Param(m.N, initialize =price_data, domain=pyo.Reals, mutable=True,
                          doc="Model price over prediction horizon")
    m.pWind     = pyo.Param(m.N, initialize=wind_data, domain=pyo.NonNegativeReals, mutable=True, 
                        doc="Model wind forecast over prediction horizon")
    m.pDemand   = pyo.Param(m.N, initialize=demand_data, domain=pyo.NonNegativeReals, mutable=True,
                          doc="Model demand over prediction horizon")
    m.E0        = pyo.Param(initialize=0, domain=pyo.NonNegativeReals, mutable=True, doc="Initial bess inventory")

    # ---------------------------- Variables ------------------------------------
    m.P_gt = pyo.Var(m.N, m.G, bounds=lambda model, t, g: (turbine_data.flow_min[g], turbine_data.flow_max[g]), 
                    domain=pyo.NonNegativeReals, 
                    doc="Power output of each device, with max bound")
    m.vGas = pyo.Var(m.N, m.G, domain=pyo.NonNegativeReals, 
                     doc="Amount of gas used at time t by device dev")
    m.vChargeRate = pyo.Var(m.N, initialize = 0.0, bounds=(0, 2), 
                            doc="Charging rate [MW], from 0 - 2")
    m.vDischargeRate = pyo.Var(m.N, initialize = 0.0, bounds=(0, 4), 
                               doc="Discharge rate [MW], from 0 - 2")
    m.E = pyo.Var(m.N, bounds=(0, 6), 
                    doc="Energy preservation from t, t+T [MW]")
    m.vGrid = pyo.Var(m.N, domain=pyo.Reals, bounds=(-20, 20), doc="Energy bought from the grid at time t, with capacity constraint")
    m.vPenalty = pyo.Var(m.N, domain=pyo.Reals, doc="This is just to map out the penalty to the solver")
    ...
    def _rule_gas_turbine_fuel_usage(model, t, dev):
        ...
        rhs = (B * (model.pDeviceOn[dev]) * P_max
            + A * model.P_gt[t, dev] )
        return lhs == rhs
    ...
    def _rule_energy_balance(model,t):
        ...
        """
        if include_grid:
            return model.pDemand[t] == -model.P_bess[t] + model.P_generator_output[t] + model.P_wind_output[t] \
                + model.vGrid[t]
        return model.pDemand[t] == -model.P_bess[t] + model.P_generator_output[t] + model.P_wind_output[t]
    ...
    def _expr_Cost_co2_released(model):
        return co2_tax * sum(model.vGas[t, dev] * gasdata["co2_content"] for dev in model.G for t in model.N)
    ...
    def _expr_Cost_quad(model):
        ...
        a, b , c = 100, 6, 0.005
        return sum(c * model.P_gt[t, dev]**2 + b * model.P_gt[t, dev] + a for t in model.N for dev in model.G)
    ...
    def penalty(model, t):
        return model.vPenalty[t] >= sum(0.005*model.P_gt[t, dev]**2 + 6 * model.P_gt[t, dev] + 100 for dev in model.G)\
        +   co2_tax * sum(model.vGas[t, dev] * gasdata["co2_content"] for dev in model.G)\
...
def build_dg_model_MPC(price_data, wind_data, 
                       demand_data, turbine_data, horizon_length, include_grid=False):
    ...
    m.pDeviceOn = pyo.Param(m.G, initialize=1, domain=pyo.Binary,mutable=False, \
                            doc="On/Off status of generator dev at time t")  
    m.pPrice    = pyo.Param(m.N, initialize =price_data[:horizon_length], domain=pyo.Reals, mutable=True,
                          doc="Model price over prediction horizon")
    m.pWind     = pyo.Param(m.N, initialize=wind_data[:horizon_length], domain=pyo.NonNegativeReals, mutable=True, 
                        doc="Model wind forecast over prediction horizon")
    m.pDemand   = pyo.Param(m.N, initialize=demand_data, domain=pyo.NonNegativeReals, mutable=True,
                          doc="Model demand over prediction horizon")
    m.E0        = pyo.Param(initialize=0, domain=pyo.NonNegativeReals, mutable=True, doc="Initial bess inventory")
    ...
    @m.Constraint(m.N, doc="Net sum of the total penalty at time t")
    def penalty(model, t):
        return model.vPenalty[t] >= sum(0.005*model.P_gt[t, dev]**2 + 6 * model.P_gt[t, dev] + 100 + turbine_data["O&M"][dev] for dev in model.G)\
        +   co2_tax * sum(model.vGas[t, dev] * gasdata["co2_content"] for dev in model.G)\
        -   100 * (model.vDischargeRate[t] - model.vChargeRate[t])\
        +   model.pPrice[t] * model.vGrid[t]
...
def run_rhc3(m: pyo.Model, 
    ...
    assert sim_steps >= horizon_length, "Number of simulations must be greater than the length of the horizon!"
    ...
    for sim_t0 in range(sim_steps - horizon_length):
        ...
        model_interface.load_data(new_forecast, time_points=new_forecast.get_time_points())
        
        if sim_t0 % nowcast_horizon == 0: 
            new_nowcast = nowcast_data.get_data_at_time(hour_6_horizon) # this is the length of the loaded 
            new_nowcast.shift_time_points(-sim_t0)
            model_interface.load_data(new_nowcast, time_points=new_nowcast.get_time_points())
        else:
        ...
        res = solver.solve(m, logfile="if-infeasible-basic.log") #, load_solutions=False)
    ...
    for sim_t0 in range(sim_steps-horizon_length, sim_steps-1):
        hour_6_horizon = [sim_t0 + t for t in range(1, min(nowcast_horizon, sim_steps - sim_t0))]
...
def run_hybrid_rhc3(m: pyo.Model, 
            horizon_length: int,
            f_data: dict, 
            n_data: dict, 
            sim_steps: int,
    ...
    for sim_t0 in range(sim_steps - horizon_length):
        ...
        if verbose:
            ...
            print(f"pDemand: {df['pDemand'].tolist()}")
            print(sim_data.to_serializable())
...
def hybrid_get_deterministic_dataframe(m: pyo.Model):
    ...
    df = pd.DataFrame({'vChargeRate': [m.vChargeRate[i].value for i in m.N], 
...
def calculate_hybridmodel_actualcost(df_res, price):
    ...
    df['vPenalty'] = df['turbine_cost'] + df['co2_cost'] - df['battery_operation'] + df['grid_cost']
    df['Cumulative Penalty'] = df['vPenalty'].cumsum()
    return df
...
def generate_variable_demand(days=7):
    ...
    for day in range(days):
        peak_hours = np.random.choice(range(day*24 + 17, day*24 + 22), 2, replace=False)
